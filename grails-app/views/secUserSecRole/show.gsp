	...
	<body>
		...
		<div id="show-secUserSecRole" class="content scaffold-show" role="main">
			...
			<ol class="property-list secUserSecRole">
				...
				<g:if test="${secUserSecRoleInstance?.sercRole}">
				<li class="fieldcontain">
					<span id="sercRole-label" class="property-label"><g:message code="secUserSecRole.sercRole.label" default="Serc Role" /></span>
					
						<span class="property-value" aria-labelledby="sercRole-label"><g:link controller="secRole" action="show" id="${secUserSecRoleInstance?.sercRole?.id}">${secUserSecRoleInstance?.sercRole?.encodeAsHTML()}</g:link></span>
